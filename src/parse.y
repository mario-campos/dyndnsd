...
domain	: DOMAIN value					{ $$ = cst_node(DOMAIN, NULL, $2, NULL); }
	| DOMAIN LUNQUOTE				{
								yyerror("%s:%d: error: no opening quote: %s", filename, $1, $2);
...
run	: RUN exprs					{
								if (++nrun > 1) yyerror("%s:%d: error: redundant statement", filename, $1);
...
value	: STRING					{ $$ = cst_node(STRING, $1, NULL, NULL); }
	| QUOTE						{ $$ = cst_node(QUOTE, $1, NULL, NULL); }
	;

expr	: STRING					{ $$ = cst_node(STRING, $1, NULL, NULL); }
	| QUOTE						{ $$ = cst_node(QUOTE, $1, NULL, NULL); }
	| LUNQUOTE					{ $$ = cst_node(LUNQUOTE, $1, NULL, NULL); }
	| RUNQUOTE					{ $$ = cst_node(RUNQUOTE, $1, NULL, NULL); }
	;
